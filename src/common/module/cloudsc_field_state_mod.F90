...
MODULE CLOUDSC_FIELD_STATE_MOD
  ...
  USE FIELD_MODULE, ONLY: FIELD_2D, FIELD_2D_OWNER, FIELD_3D, FIELD_3D_WRAPPER_PACKED, FIELD_3D_OWNER, FIELD_4D, FIELD_4D_OWNER, FIELD_INT2D, FIELD_INT2D_OWNER, FIELD_LOG2D, FIELD_LOG2D_OWNER, MALLOC_HOST_PINNED_4D
...
!...Device pointers for bulk allocations
    REAL(KIND=JPRB), DEVICE, ALLOCATABLE :: DEVDATA_RDONLY(:,:,:,:)
    REAL(KIND=JPRB), DEVICE, ALLOCATABLE :: DEVDATA_RWONLY(:,:,:,:)
...
CONTAINS
  ...
  SUBROUTINE COPY_BULK_STORAGE_TO_DEVICE(ARR, DEVDATA, DEVPTR)
    REAL(KIND=JPRB), INTENT(IN) :: ARR(:,:,:,:)
    REAL(KIND=JPRB), DEVICE, ALLOCATABLE, INTENT(INOUT) :: DEVDATA(:,:,:,:) 
    REAL(KIND=JPRB), POINTER, CONTIGUOUS, INTENT(INOUT) :: DEVPTR(:,:,:,:) 
    INTEGER(KIND=JPIM) :: ARRSIZE
    TYPE(C_PTR) :: HPTR

    ALLOCATE(DEVDATA, MOLD=ARR)

    ARRSIZE = SIZE(ARR) * SIZEOF(1.0_JPRB)
    CALL ACC_MAP_DATA(ARR, DEVDATA, ARRSIZE)
    CALL ACC_MEMCPY_TO_DEVICE(DEVDATA(:,:,:,:), ARR(:,:,:,:), ARRSIZE)

    HPTR = ACC_HOSTPTR(DEVDATA)
    CALL C_F_POINTER(HPTR, DEVPTR, SHAPE(DEVDATA))

  END SUBROUTINE COPY_BULK_STORAGE_TO_DEVICE

  SUBROUTINE COPY_BULK_STORAGE_TO_HOST(ARR, DEVDATA)
    ...
    CALL ACC_MEMCPY_FROM_DEVICE(ARR(:,:,:,:), DEVDATA(:,:,:,:), ARRSIZE)
    CALL ACC_UNMAP_DATA(ARR)
 
    DEALLOCATE(DEVDATA)
  END SUBROUTINE COPY_BULK_STORAGE_TO_HOST
  ...
  FUNCTION CREATE_FIELD_WRAP_PACKED_3D(DATA, IDX) RESULT(FIELD_PTR)
    ! Create a single 3D field with implicit blocking dimension by wrapping existing data
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    ...
    CLASS(FIELD_2D), INTENT(INOUT) :: FIELD
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_INT2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    CHARACTER(LEN=*), INTENT(IN) :: NAME
    CLASS(FIELD_INT2D), INTENT(INOUT) :: FIELD
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NPROMA, NGPTOT, NBLOCKS
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_LOG2D(NAME, FIELD, NLON, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    CHARACTER(LEN=*), INTENT(IN) :: NAME
    CLASS(FIELD_LOG2D), INTENT(INOUT) :: FIELD
    INTEGER(KIND=JPIM), INTENT(IN) :: NLON, NPROMA, NGPTOT, NBLOCKS
    INTEGER(KIND=JPIM), INTENT(IN), OPTIONAL :: NGPTOTG
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_3D(NAME, FIELD, NLON, NLEV, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    CHARACTER(LEN=*), INTENT(IN) :: NAME
    CLASS(FIELD_3D), INTENT(INOUT) :: FIELD
  ...
  SUBROUTINE LOAD_AND_EXPAND_FIELD_4D(NAME, FIELD, NLON, NLEV, NDIM, NPROMA, NGPTOT, NBLOCKS, NGPTOTG)
    CHARACTER(LEN=*), INTENT(IN) :: NAME
    CLASS(FIELD_4D), INTENT(INOUT) :: FIELD
...
!$omp end parallel do 
  ...
  SUBROUTINE CLOUDSC_FIELD_STATE_LOAD(SELF, NPROMA, NGPTOT, NGPTOTG, USE_PACKED)
    ...
    IF (LLPACKED) THEN
      ...
      SELF%F_PSNDE => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=14)
      SELF%F_PMFU => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=15)
      SELF%F_PMFD => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=16)
      SELF%F_PA => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=17)
      SELF%F_PSUPSAT => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=18)
      SELF%F_PLCRIT_AER => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=19)
      SELF%F_PICRIT_AER => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=20)
      SELF%F_PRE_ICE => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=21)
      SELF%F_PCCN => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=22)
      SELF%F_PNICE => CREATE_FIELD_WRAP_PACKED_3D(DATA=SELF%DATA_RDONLY, IDX=23)
      ...
      NFIELDS = 14
!      ALLOCATE(SELF%DATA_RWONLY(NPROMA, SELF%KLEV+1, NFIELDS, SELF%NBLOCKS))
